...
instead, we report 95\% confidence intervals estimated using the bootstrap
method.

%% TODO(agy): add the figure from the previous paragraph, import the two
%%*_scatter.pdf charts

%% The description text should be: Trial-by-trial execution time for two
%% benchmarks (\texttt{delunay} and \texttt{dedup}) for the MLton compiler. The
%% experimental side is the \texttt{tuple}-flattening configuration in both cases.
...
The effect is modest across the board on this benchmark suite: when the pass has
any impact at all, it duplicates functions, leading to 
