...
\begin{figure}[htbp]
  ...
  \end{subfigure}
  \caption{Trial-by-trial execution time for two benchmarks (\texttt{delunay} and \texttt{dedup}) for the MLton compiler. The experimental side is the \texttt{tuple}-flattening configuration for \texttt{delunay} and \textt{ConApp}-flattening for \texttt{dedup}.}
...
%% TODO(agy): add in the reverb_scatter figure (find it under charts/)

%% use the description: Trial-by-trial execution time for \texttt{reverb} in the
%% \textt{tuple}-flattening configuration.
