...
%% Fix incorrect rendering for <> in figure labels
...
\subsection{Comparison to more-agressive \texttt{DeepFlatten}}
...
\item \texttt{parallel-ml-bench}: an MPL-specific benchmark suite introduced as
  part of \cite{mpl-orig}. This benchmark suite is focused largely on
...
side'' (the ``good'' outcome) and >1.0 means ``bigger number on the
test side (the ``bad'' outcome).
...
\hl{TODO: THIS IS ON BAD GCP HW! NEED TO RERUN}

\hl{TODO: MORE RUNS TO CAPTURE NOISE?}

\hl{TODO: FORMATTING IS BAD, HOW TO CONDENSE?}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.8\textwidth]{../analysis/charts/tuple_mlton_run_mlton_vs_mlton.pdf}
  \caption{Relative run time for with- vs without-\texttt{PreFlatten}. <0\% represents an improvement, >0\% represents a regression.}
  \label{fig:tuple_mlton_run_mlton_vs_mlton}
\end{figure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.8\textwidth]{../analysis/charts/tuple_mlton_compile_mlton_vs_mlton.pdf}
  \caption{Relative compile time for with- vs without-\texttt{PreFlatten}. <0\% represents an improvement, >0\% represents a regression.}
  \label{fig:tuple_mlton_compile_mlton_vs_mlton}
\end{figure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.8\textwidth]{../analysis/charts/tuple_mlton_size_mlton_vs_mlton.pdf}
  \caption{Relative binary size for with- vs without-\texttt{PreFlatten}. $<0$\% represents an improvement, $>0$\% represents a regression.}
  \label{fig:tuple_mlton_size_mlton_vs_mlton}
\end{figure}

\hl{TODO: THIS SEEMS LIKE A NOISY RUN -- NEED TO REDO. IGNORING THE DLX SIMULATOR RESULT.}

As we can see in \cref{fig:tuple_mlton_run_mlton_vs_mlton}, we get a small, <5\%
win on the \texttt{count-graphs} benchmark, and the other tests are basically
neutral. \Cref{fig:tuple_mlton_compile_mlton_vs_mlton} shows that compile times
are broadly similar, a 1\% geomean regression across the changed benchmarks in
the benchmark suite. As expected, \cref{fig:tuple_mlton_size_mlton_vs_mlton}
shows a strict increase in binary size: a geomean 1.8\% regression on impacted
benchmarks.

\subsubsection{\texttt{parallel-ml-bench}: MLton vs MLton}

\hl{TODO: DESCRIBE CONFIG, ANALYSIS}

\hl{TODO: SKIPPED COMPILE SIZE}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.8\textwidth]{../analysis/charts/tuple_parallel_bench_run_mlton_vs_mlton.pdf}
  \caption{Relative run time for with- vs without-\texttt{PreFlatten}, applied to tuple constructors. <0\% represents an improvement, >0\% represents a regression. Targets the \texttt{parallel-ml-bench} benchmark suite.}
  \label{fig:tuple_parallel_bench_run_mlton_vs_mlton}
\end{figure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.8\textwidth]{../analysis/charts/tuple_parallel_bench_size_mlton_vs_mlton.pdf}
  \caption{Relative binary size for with- vs without-\texttt{PreFlatten}, applied to tuple constructors. <0\% represents an improvement, >0\% represents a regression. Targets the \texttt{parallel-ml-bench} benchmark suite.}
...
\hl{TODO: NEED TO INVESTIGATE BIG REGRESSION: SEEMS REAL}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.8\textwidth]{../analysis/charts/con_mlton_run_mlton_vs_mlton.pdf}
  \caption{Relative run time for with- vs without-\texttt{PreFlatten}, applied to \texttt{ConApp}. <0\% represents an improvement, >0\% represents a regression.}
  \label{fig:con_mlton_run_mlton_vs_mlton}
\end{figure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.8\textwidth]{../analysis/charts/con_mlton_compile_mlton_vs_mlton.pdf}
  \caption{Relative compile time for with- vs without-\texttt{PreFlatten}, applied to \texttt{ConApp}. <0\% represents an improvement, >0\% represents a regression.}
  \label{fig:con_mlton_compile_mlton_vs_mlton}
\end{figure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.8\textwidth]{../analysis/charts/con_mlton_size_mlton_vs_mlton.pdf}
  \caption{Relative binary size for with- vs without-\texttt{PreFlatten}, applied to \texttt{ConApp}. <0\% represents an improvement, >0\% represents a regression.}
  \label{fig:con_mlton_size_mlton_vs_mlton}
\end{figure}

\hl{TODO: SUSPECT THE MEASUREMENTS ABOVE WERE IN A BAD STATE. NEED TO RERUN ON CHAMELEON}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.8\textwidth]{../analysis/charts/con_parallel_bench_run_mlton_vs_mlton.pdf}
  \caption{Relative run time for with- vs without-\texttt{PreFlatten}, applied to \texttt{ConApp}. <0\% represents an improvement, >0\% represents a regression. Targets the \texttt{parallel-ml-bench} benchmark suite.}
  \label{fig:con_parallel_bench_run_mlton_vs_mlton}
\end{figure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.8\textwidth]{../analysis/charts/con_parallel_bench_size_mlton_vs_mlton.pdf}
  \caption{Relative binary size for with- vs without-\texttt{PreFlatten}, applied to \texttt{ConApp}. <0\% represents an improvement, >0\% represents a regression. Targets the \texttt{parallel-ml-bench} benchmark suite.}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.8\textwidth]{../analysis/charts/aos_mlton_run_mlton_vs_mlton.pdf}
  \caption{Relative run time for with- vs without-\texttt{ShallowFlatten}, using the AoS transformation. <0\% represents an improvement, >0\% represents a regression.}
  \label{fig:aos_mlton_run_mlton_vs_mlton}
\end{figure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.8\textwidth]{../analysis/charts/aos_mlton_compile_mlton_vs_mlton.pdf}
  \caption{Relative compile time for with- vs without-\texttt{ShallowFlatten}, using the AoS transformation. <0\% represents an improvement, >0\% represents a regression.}
  \label{fig:aos_mlton_compile_mlton_vs_mlton}
\end{figure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.8\textwidth]{../analysis/charts/aos_mlton_size_mlton_vs_mlton.pdf}
  \caption{Relative binary size for with- vs without-\texttt{ShallowFlatten}, using the AoS transformation. <0\% represents an improvement, >0\% represents a regression.}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.8\textwidth]{../analysis/charts/aos_parallel_bench_run_mlton_vs_mlton.pdf}
  \caption{Relative run time for with- vs without-\texttt{ShallowFlatten}, using the AoS transformation. <0\% represents an improvement, >0\% represents a regression. Targets the \texttt{parallel-ml-bench} benchmark suite.}
  \label{fig:aos_parallel_bench_run_mlton_vs_mlton}
\end{figure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.8\textwidth]{../analysis/charts/aos_parallel_bench_size_mlton_vs_mlton.pdf}
  \caption{Relative binary size for with- vs without-\texttt{ShallowFlatten}, using the AoS transformation. <0\% represents an improvement, >0\% represents a regression. Targets the \texttt{parallel-ml-bench} benchmark suite.}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.8\textwidth]{../analysis/charts/soa_mlton_run_mlton_vs_mlton.pdf}
  \caption{Relative run time for with- vs without-\texttt{ShallowFlatten}, using the SoA transformation. <0\% represents an improvement, >0\% represents a regression.}
  \label{fig:soa_mlton_run_mlton_vs_mlton}
\end{figure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.8\textwidth]{../analysis/charts/soa_mlton_compile_mlton_vs_mlton.pdf}
  \caption{Relative compile time for with- vs without-\texttt{ShallowFlatten}, using the SoA transformation. <0\% represents an improvement, >0\% represents a regression.}
  \label{fig:soa_mlton_compile_mlton_vs_mlton}
\end{figure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.8\textwidth]{../analysis/charts/soa_mlton_size_mlton_vs_mlton.pdf}
  \caption{Relative binary size for with- vs without-\texttt{ShallowFlatten}, using the SoA transformation. <0\% represents an improvement, >0\% represents a regression.}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.8\textwidth]{../analysis/charts/soa_parallel_bench_run_mlton_vs_mlton.pdf}
  \caption{Relative run time for with- vs without-\texttt{ShallowFlatten}, using the SoA transformation. <0\% represents an improvement, >0\% represents a regression. Targets the \texttt{parallel-ml-bench} benchmark suite.}
  \label{fig:soa_parallel_bench_run_mlton_vs_mlton}
\end{figure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.8\textwidth]{../analysis/charts/soa_parallel_bench_size_mlton_vs_mlton.pdf}
  \caption{Relative binary size for with- vs without-\texttt{ShallowFlatten}, using the SoA transformation. <0\% represents an improvement, >0\% represents a regression. Targets the \texttt{parallel-ml-bench} benchmark suite.}
...
\cite{flat-paper}, but extends it to a) handle additional cases where the
...
\cite{mlton-closure-paper} introduced the MLton defunctionalization pass, whose \texttt{datatype} objects we seek to eliminate through our \texttt{PreFlatten} pass. We, effectively, repurpose their control flow analysis in order to drive function specialization decisions.

\cite{local-paper} and \cite{adt4hpc-paper} aim to achieve a similar goal to our \texttt{ConApp} flattening (eliminating indirections while traversing ADTs), but address it in a different context: they are concerned with manual control over the format of serialized data on disk, rather than (like us) specializing the in-memory format of data based on its users (while, jointly, specializing users according to the in-memory format)

\cite{double-steal-paper} also investigates a similar problem to our \texttt{ConApp} flattening (eliminating pointers introduced by ADTs in the source language); however their contribution focuses on a novel, lossless in-memory representation for the ADT rather than optimize-out the ADT metadata entirely.

\cite{bbv-paper} applies similar techniques to our \texttt{ConApp} flattening (i.e. emitting specialized code based on callers) in the context of optimizing a dynamically-typed language. Our work differs from theirs in that we specialize only on properties that we can guarantee from the type system, and so both our policy decisions and our implementation mechanism are greatly simplified (the latter relying entirely on standard, pre-existing optimization techniques to "clean up" in the specialized blocks).

\cite{manticore-arity-paper} treats a problem very similar to \cite{flat-paper}. Much of their complexity stems from performing the transformation on an higher-order IR, whereas we (like \cite{flat-paper}) target a first-order IR, and so allow for a simpler analysis. They also provide a flattening heuristic that is maximally conservative in the sense that it never introduces unnecessary pack/unpack operations, whereas we aim to explore more aggressive tradeoffs.

The recent work \cite{closure-sharing-paper} examines the opposite optimization strategy to the one discussed here – introducing new layers of indirection in closure representations as a performance optimization. Like \cite{manticore-arity-paper}, their analysis differs substantially from anything in MLton, since MLton implements higher-order functions via defunctionalization rather than closure conversion.
...
\cite{dataonly-paper} applies a similar transformation (tuple flattening) in a similar context: in fact, their "data-only flattening" is roughly the SoA transformation that we apply. Our SoA transformation differs from theirs by lacking shape descriptors. We differ from their approach by a) considering a broader class of flattening strategies, and b) applying our pass in tandem with the existing flattening passes to eliminate intermediate tuples, rather than relying on a single huge pass to do the fusion. i.e. we have a simpler transformation by operating at a higher level of abstraction in the compiler and leveraging pre-existing optimizations

Various work at jane street for unboxed types in OCaml aims to materialize the same wins that we demonstrate here through the SoA transformation, as discussed in e.g. \cite{layout-poly-paper}. However, the primary technical challenge that they seek to overcome – selecting the appropriate function specialization for a potentially-unboxed type – does not apply to MPL: since MLton does whole-program compilation, we always know at compile time which specializations are necessary to emit.

\cite{futhark-flatten} also discusses the problem of choosing an optimum flattened representation from among the possible choices – here on the GPU for the Futhark programming language. Their problem differs from ours in that MPL resolves the issue of work imbalance between functional units (CPU cores or GPU SMs) through low-overhead scheduling of tasks at runtime, rather than at compile time, as explored in \cite{mpl-sched-paper}.

\cite{realtime-sml-paper} also investigates the impact of deep flattening optimizations on performance: the authors there disable \texttt{DeepFlatten} in order to avoid complexity when computing byte offsets for post-\texttt{DeepFlatten} arrays whose flattened tuples contained elements of different sizes. We avoid this issue by simply restricting ourselves to tuples with uniform element types and observe that this is sufficient for all opportunities in our benchmark suites.
...
\item Lit review
  ...
  \item Don't use raw /cite -- the sentences should read like the footnote
  ...
  \item Consider using multiple /subsection or /paragraph blocks
