...
further by examinining \hl{TODO: finish}

\hl{TODO: dig into the mispredict data}
...
\hl{TODO(agy): Add the charts ``tuple exp bfs'' and ``tuple exp delaunay'' side
  by side in a figure with the label fig:tuple\_standouts\_only}

\hl{TODO(agy): Add the chart ``tuple exp geomeans'' in a figure with the name
  fig:tuple\_exp\_geomeans}

\hl{TODO(agy): Add the chart Tuple chunkify one run time comparison across core
  counts (trellis) in a figure with the name fig:tuple\_all\_chunkify\_one}.
