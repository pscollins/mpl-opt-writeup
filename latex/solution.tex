...
The basic intution is that if we see a sequence like:
\begin{minted}{python}
  def f(...) -> ...:
    ...other blocks...
  ...
  def g(t: 'a * 'b) -> ...:
    ...
    ...other blocks...
\end{minted}
then we can elimiate the intermediate tuple \texttt{t} with a simple peephole
...
perspective of efficiently utilizing the memory controler, in terms of both
...
and AoS cases. Recall from above that this transformation is applied iteratively
until convergence; below we will outline only a single iteration for
convencience.

Beginning with the SoA case, the general form of the transformation is ``
