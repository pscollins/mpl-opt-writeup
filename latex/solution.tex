...
\cite{futhark-flatten} also discusses the problem of choosing an optimum flattened representation from among the possible choices – here on the GPU for the Futhark programming language. Their problem differs from ours in that MPL resolves the issue of work imbalance between functional units (CPU cores or GPU SMs) through clever low-overhead scheduling of tasks at runtime, rather than at compile time, as explored in \cite{mpl-sched-paper}.
