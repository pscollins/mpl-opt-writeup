...
\begin{itemize}
\item Organize the evaluation in terms of the takeways that we'd like to tell
  (i.e. ``it works when you fix chunkify'') rather than in terms of the process
  of the investigation
\item Should do some investigation to figure out why chunkification actually
  fails -- maybe there's a silly mechanism like ``location of the function
  definition'' that matters
\item Forbid nested case arms (done); think of AST rather than parser (done)
\item Check on the benchmark count strategy with parallel ML bench -- it should
  be using fixed iters and not fixed time
\end{itemize}
...
\begin{itemize}
\item Should probably rename \texttt{PreFlatten} to something more intuitive
\item Does it look bad to use LLM-generated diagrams?
\end{itemize}
