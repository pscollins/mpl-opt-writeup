...
Instead, we will work here with a loose, Python-inspired pseudocode (whose
structure mirrors the real SSA IR), and assert that the \texttt{fib} function is
lowered to \texttt{SSA} as the snippet in \cref{fig:fib_ssa}.

\begin{figure}[htbp]
  \centering
  \begin{subfigure}[t]{0.33\textwidth}
    \caption{Standard ML source}
    \label{fig:fib_sml}
    \vspace{1mm}
\begin{minted}[fontsize=\footnotesize,frame=single,framesep=2.5mm,rulecolor=\color{black!20}]{sml}
fun fib(n) =
  if n = 0 then 1
  else n * fib(n - 1)
...
\end{minted}
  ...
  \begin{subfigure}[t]{0.64\textwidth}
    \caption{Lowered \texttt{SSA} pseudocode}
    \label{fig:fib_ssa}
    \vspace{1mm}
\begin{minted}[fontsize=\footnotesize,frame=single,framesep=2.5mm,rulecolor=\color{black!20}]{python}
def fib(n: int): int =
  block bb0():
...
\end{minted}
  \end{subfigure}
  \caption{A simple \texttt{fib} program in Standard ML alongside its representation lowered to \texttt{SSA} (in pseudocode).}
  \label{fig:fib_comparison}
\end{figure}

%% \hl{TODO(pscollins): fix naming for SSA}

%% \hl{TODO: grammar is misisng constants, also missing tuple/container types. What
%%   do we call the SSA?}

A grammar for this intermediate representation is provided in
\cref{fig:ssa_grammar}. Mirroring MLton's representation of \texttt{SSA}, a
program consists of datatype declarations, global statement bindings, and a
collection of first-order functions.

Each function body is represented as a control flow graph of basic blocks
carrying (optional) block arguments. MLton uses block arguments combined with
dominator-based scoping to replace the $\phi$ nodes used in other variants of
SSA: bound variables are available for use in any program position that they
strictly dominate, and conditionally binding variables to names is implemented
with a conditional \texttt{goto}.

The \texttt{call} statement differs somewhat from the \texttt{call} statement
used in other SSA IRs: the syntax is \texttt{call return\_label func\_name
  (...arguments...)}, where \texttt{return\_label} is a block label within the
current function. Names for function return values in the caller are thus
bound using the same block-argument mechanism described above.

Concrete operations on particular types are implemented in MLton as ``primitive
applications,'' denoted in the IR by the keyword \texttt{prim}. MLton defines on
the order of 500 distinct \texttt{prim}s, implemeting arithmetic operations,
container manipulation, casts, and so on. Different \texttt{prim}s are
eliminated in different compiler passes by either a decomposition into simpler
\texttt{prim}s or a direct C call. MLton helpfully provides a type-checker for
\texttt{prim}s in each IR, ensuring that the program remains well-formed after
each compiler pass.

The semantics of \texttt{SSA} are otherwise broadly the same as their usual SML
equivalents: \texttt{datatype} introduces new sum type endowed with some set of
constructors, \texttt{con} represents the application of one such constructor to
concrete arguments, \texttt{case} implements conditional jumps with pattern
matching, and \texttt{tuple} has some special syntactic support in the form of
the \texttt{select} operator. The types \texttt{a' array} and \texttt{a' vector}
likewise match their usual SML counterparts, representing a mutable or an
immutable container of objects of type \texttt{a'}, respectively.

\begin{figure}[htbp]
  \centering
  \small
  \begin{align*}
    \langle\textit{program}\rangle &\Coloneqq \langle\textit{decl}\rangle^+ \\[0.3em]
    \langle\textit{decl}\rangle &\Coloneqq \langle\textit{datatype\_def}\rangle \mid \langle\textit{stmt}\rangle \mid \langle\textit{function\_def}\rangle \\[0.3em]
    \langle\textit{datatype\_def}\rangle &\Coloneqq \texttt{datatype} \;\langle\textit{id}\rangle \mathrel{\texttt{=}} \langle\textit{con\_def}\rangle \;(\texttt{|} \;\langle\textit{con\_def}\rangle)^* \\[0.3em]
    \langle\textit{con\_def}\rangle &\Coloneqq \langle\textit{id}\rangle \;[\texttt{of} \;\langle\textit{type}\rangle] \\[0.3em]
    \langle\textit{function\_def}\rangle &\Coloneqq \texttt{def} \;\langle\textit{id}\rangle \;\texttt{(} \,[\langle\textit{param\_list}\rangle]\, \texttt{)} \mathbin{\texttt{:}} \langle\textit{type}\rangle \mathrel{\texttt{=}} \langle\textit{block}\rangle^+ \\[0.3em]
    \langle\textit{param\_list}\rangle &\Coloneqq \langle\textit{param}\rangle \;(\texttt{,} \;\langle\textit{param}\rangle)^* \\[0.3em]
    \langle\textit{param}\rangle &\Coloneqq \langle\textit{id}\rangle \mathbin{\texttt{:}} \langle\textit{type}\rangle \\[0.3em]
    \langle\textit{block}\rangle &\Coloneqq \texttt{block} \;\langle\textit{id}\rangle \;\texttt{(} \,[\langle\textit{param\_list}\rangle]\, \texttt{)} \mathbin{\texttt{:}} \langle\textit{stmt}\rangle^* \;\langle\textit{transfer}\rangle \\[0.3em]
    \langle\textit{stmt}\rangle &\Coloneqq (\langle\textit{id}\rangle \mathbin{\texttt{:}} \langle\textit{type}\rangle \mid \texttt{\_}) \mathrel{\texttt{=}} \langle\textit{exp}\rangle \\[0.3em]
    \langle\textit{exp}\rangle &\Coloneqq \langle\textit{operand}\rangle \\
    &\quad \mid \texttt{prim} \;\langle\textit{id}\rangle \,[\texttt{[}\langle\textit{type\_list}\rangle\texttt{]}] \;\texttt{(} \,[\langle\textit{arg\_list}\rangle]\, \texttt{)} \\
    &\quad \mid \texttt{tuple} \;\texttt{(} \,[\langle\textit{arg\_list}\rangle]\, \texttt{)} \mid \texttt{(} \;\langle\textit{operand}\rangle \;(\texttt{,} \;\langle\textit{operand}\rangle)^+ \;\texttt{)} \\
    &\quad \mid \texttt{select} \;\texttt{(} \;\langle\textit{operand}\rangle \;\texttt{,} \;\langle\textit{integer}\rangle \;\texttt{)} \\
    &\quad \mid \texttt{con} \;\langle\textit{id}\rangle \;[\texttt{(} \,[\langle\textit{arg\_list}\rangle]\, \texttt{)}] \\[0.3em]
    \langle\textit{transfer}\rangle &\Coloneqq \texttt{return} \;\langle\textit{operand}\rangle \\
    &\quad \mid \texttt{goto} \;\langle\textit{id}\rangle \;[\texttt{(} \,[\langle\textit{arg\_list}\rangle]\, \texttt{)}] \\
    &\quad \mid \texttt{call} \;\langle\textit{id}\rangle \;\langle\textit{id}\rangle \;\texttt{(} \,[\langle\textit{arg\_list}\rangle]\, \texttt{)} \\
    &\quad \mid \texttt{case} \;\langle\textit{operand}\rangle \;\texttt{of} \;\langle\textit{case\_arm}\rangle \;(\texttt{|} \;\langle\textit{case\_arm}\rangle)^* \\[0.3em]
    \langle\textit{case\_arm}\rangle &\Coloneqq \langle\textit{pattern}\rangle \mathrel{\texttt{=>}} \texttt{goto} \;\langle\textit{id}\rangle \;[\texttt{(} \,[\langle\textit{arg\_list}\rangle]\, \texttt{)}] \\[0.3em]
    \langle\textit{pattern}\rangle &\Coloneqq \langle\textit{id}\rangle \mid \langle\textit{const}\rangle \mid \texttt{\_} \\[0.3em]
    \langle\textit{type}\rangle &\Coloneqq \langle\textit{id}\rangle \mid \langle\textit{tuple\_type}\rangle \mid \langle\textit{type}\rangle \;\texttt{array} \mid \langle\textit{type}\rangle \;\texttt{vector} \mid \texttt{(} \;\langle\textit{type}\rangle \;\texttt{)} \\[0.3em]
    \langle\textit{tuple\_type}\rangle &\Coloneqq \langle\textit{type}\rangle \;(\mathbin{\texttt{*}} \;\langle\textit{type}\rangle)^+ \\[0.3em]
    \langle\textit{type\_list}\rangle &\Coloneqq \langle\textit{type}\rangle \;(\texttt{,} \;\langle\textit{type}\rangle)^* \\[0.3em]
    \langle\textit{arg\_list}\rangle &\Coloneqq \langle\textit{operand}\rangle \;(\texttt{,} \;\langle\textit{operand}\rangle)^* \\[0.3em]
    \langle\textit{operand}\rangle &\Coloneqq \langle\textit{id}\rangle \mid \langle\textit{const}\rangle \\[0.3em]
    \langle\textit{const}\rangle &\Coloneqq \langle\textit{integer}\rangle \mid \langle\textit{float}\rangle \mid \texttt{true} \mid \texttt{false} \mid \texttt{()} \\[0.3em]
    \langle\textit{id}\rangle &\Coloneqq \texttt{[a-zA-Z\_'][a-zA-Z0-9\_']*} \\[0.3em]
    \langle\textit{integer}\rangle &\Coloneqq \texttt{[0-9]+} \\[0.3em]
    \langle\textit{float}\rangle &\Coloneqq \texttt{[0-9]+\textbackslash.[0-9]+}
  \end{align*}
  \caption{An abstract syntax grammar for \texttt{SSA}.}
  \label{fig:ssa_grammar}
\end{figure}

%% \hl{TODO: collapse this section into the previous section about the grammar}
...
The core problems that we need to solve in order to translate from \texttt{SSA}
to C are:
\begin{itemize}
\item \hl{TODO: SUMMARIZE}
\end{itemize}

\hl{TODO: LET'S START OFF WITH A DIAGRAM THAT SHOWS fib CONTROL FLOW}

\hl{TODO: CLEAN UP STARTING BELOW}
