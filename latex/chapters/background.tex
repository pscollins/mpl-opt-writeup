...
Instead, we will work here with a loose, Python-inspired pseudocode, and assert
that the \texttt{fib} function is lowered to \texttt{SSA} as the the snippet in
\cref{fig:fib_ssa}.
...
%% TODO(agy): put this side-by-side in a box with the figure above
\begin{minted}{python}
  def fib(n: int): int =
    block bb0():
