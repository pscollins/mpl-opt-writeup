...
A grammar for this language is provided in \cref{fig:ssa_grammar}. Mirroring
MLton's representation of \texttt{SSA}, this language 

\hl{TODO(agy): review all uses of ``pseduo-ssa'' in this document (across all
  chapters) and update the grammar below to cover the full set of cases
  required. Formulate the grammar for the purposes of reasoning about the AST
  rather than as a parser -- we don't care about whitespace, for example}
  
\begin{figure}
  \begin{align*}
\langle\textit{program}\rangle &\Coloneqq \langle\textit{function\_def}\rangle^+ \\[0.4em]
\langle\textit{function\_def}\rangle &\Coloneqq \texttt{def} \;\langle\textit{id}\rangle \;\texttt{(} \,[\langle\textit{param\_list}\rangle]\, \texttt{)} \;\texttt{:} \;\langle\textit{type}\rangle \;\texttt{=} \;\textsc{newline} \\
&\qquad \textsc{indent} \;\langle\textit{block}\rangle^+ \;\textsc{dedent} \\[0.4em]
\langle\textit{param\_list}\rangle &\Coloneqq \langle\textit{param}\rangle \;(\texttt{,} \;\langle\textit{param}\rangle)^* \\[0.4em]
\langle\textit{param}\rangle &\Coloneqq \langle\textit{id}\rangle \;\texttt{:} \;\langle\textit{type}\rangle \\[0.4em]
\langle\textit{type}\rangle &\Coloneqq  \langle\textit{id}\rangle \\[0.4em]
\langle\textit{block}\rangle &\Coloneqq \texttt{block} \;\langle\textit{id}\rangle \;\texttt{(} \,[\langle\textit{param\_list}\rangle]\, \texttt{)} \;\texttt{:} \;\textsc{newline} \\
&\qquad \textsc{indent} \;\langle\textit{stmt}\rangle^* \;\langle\textit{terminator}\rangle \;\textsc{newline} \;\textsc{dedent} \\[0.4em]
\langle\textit{stmt}\rangle &\Coloneqq \langle\textit{id}\rangle \;\texttt{:} \;\langle\textit{type}\rangle \mathrel{\texttt{=}} \langle\textit{rhs\_expr}\rangle \;\textsc{newline} \\[0.4em]
\langle\textit{rhs\_expr}\rangle &\Coloneqq \langle\textit{operand}\rangle \\
&\quad \mid \texttt{prim} \;\langle\textit{id}\rangle \;\texttt{(} \,[\langle\textit{arg\_list}\rangle]\, \texttt{)} \\[0.4em]
\langle\textit{terminator}\rangle &\Coloneqq \texttt{return} \;\langle\textit{operand}\rangle \\
&\quad \mid \texttt{goto} \;\langle\textit{id}\rangle \;[\texttt{(} \,[\langle\textit{arg\_list}\rangle]\, \texttt{)}] \\
&\quad \mid \texttt{call} \;\langle\textit{id}\rangle \;\texttt{(} \;\langle\textit{id}\rangle \;\texttt{(} \,[\langle\textit{arg\_list}\rangle]\, \texttt{)} \;\texttt{)} \\
&\quad \mid \texttt{case} \;\langle\textit{operand}\rangle \;\texttt{of} \;\langle\textit{case\_arms}\rangle \\[0.4em]
\langle\textit{case\_arms}\rangle &\Coloneqq \langle\textit{case\_arm}\rangle \;(\texttt{|} \;\langle\textit{case\_arm}\rangle)^* \\[0.4em]
\langle\textit{case\_arm}\rangle &\Coloneqq \langle\textit{pattern}\rangle \mathrel{\texttt{=>}} \langle\textit{terminator}\rangle \\[0.4em]
\langle\textit{pattern}\rangle &\Coloneqq \langle\textit{integer}\rangle \mid \langle\textit{id}\rangle \\[0.4em]
\langle\textit{arg\_list}\rangle &\Coloneqq \langle\textit{operand}\rangle \;(\texttt{,} \;\langle\textit{operand}\rangle)^* \\[0.4em]
\langle\textit{operand}\rangle &\Coloneqq \langle\textit{id}\rangle \mid \langle\textit{integer}\rangle \mid \texttt{true} \mid \texttt{false} \\[0.4em]
\langle\textit{id}\rangle &\Coloneqq \texttt{[a-zA-Z\_][a-zA-Z0-9\_]*} \\[0.4em]
\langle\textit{integer}\rangle &\Coloneqq \texttt{[0-9]+}
\end{align*}
  \label{fig:ssa_grammar}

  \caption{A grammar for \texttt{SSA}}
\end{figure}
